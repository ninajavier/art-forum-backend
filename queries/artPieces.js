...
const deleteArtPiece = async (id) => {
  try {
    const deletedArtPiece = await db.one(
      "DELETE FROM art_pieces where id=$1 RETURNING *",
      id
    );
...
const updateArtPiece = async (id, artPiece) => {
  try {
    const updatedArtPiece = await db.one(
      "UPDATE art_pieces SET title=$1, artist=$2, medium=$3, year_created=$4, image_url=$5, is_favorite=$6 WHERE id=$7 RETURNING *",
      [
        artPiece.title,
        artPiece.artist,
        artPiece.medium,
    ...
    console.error("Error creating art piece:", e);
    return e;
  }
};
